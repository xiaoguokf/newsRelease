          ...
          >
            ...
            <div class="OwO-body">
              <ul class="OwO-items OwO-items-show">
                ...
                >
                  <img :src="require('../../assets/img_/face/' + oitem.url)" alt="" />
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    analyzeEmoji: function (cont) {
      ...
      if (content) {
        for (var i = 0; i < content.length; i++) {
          ...
          var s = require("../../assets/img_/face/" + src);
          var imoj = "<img src='" + s + "'/>";
    ...
    myaxios(method,url,type,data,id){
      ...
      this.axios({
        ...
        if(response.data.code===200){
          if(type=='deleteFirst'||type=='deleteSecond'){
            location.reload()
            // const com = this.commentList.filter(item => item.id == id)
            // console.log(com)
            ...
            that.commentList.forEach(item => {
              if(item.id==id) {
                item.like.isLike=!item.like.isLike
                  if(method=='put') {
                    item.like.likeId=response.data.data
                    item.like.likeNum +=1
                  }else{
                    item.like.likeId=null
                    item.like.likeN-=1
                  }
                }
            })
          }else if(type=='likeS'){
            that.commentList.forEach(i => {
              console.log(i)
              i.childrenList.forEach(item => {
                if(item.id==id) {
                  item.like.isLike=!item.like.isLike
                  if(method=='put') {
                    item.like.likeId=response.data.data
    ...
    likeCom(mes,id,type,likeid){
      if(type=='first'){
        if(mes=='put'){
          this.myaxios("put","/api/like/giveLike?type=2&Id="+id,"likeF",id)
        }else{
          this.myaxios("delete","/api/like/cancelLike?type=2&likeId="+likeid,"likeF",id)
        }
      }else{
        if(mes=='put'){
          this.myaxios("put","/api/like/giveLike?type=3&Id="+id,"likeS",id)
        }else{
